...
const char* gpSzAppName = "VK-DEPTH";
const wchar_t* windowName = L"DEPTH";
...
VkResult Resize(int width, int height)
{
	...
	//Destory
	{
		...
		if (vkImageViewDepth)
		{
			vkDestroyImageView(vkDevice, vkImageViewDepth, nullptr);
			vkImageViewDepth = VK_NULL_HANDLE;
		}

		if (vkDeviceMemoryDepth)
		{
			vkFreeMemory(vkDevice, vkDeviceMemoryDepth, nullptr);
			vkDeviceMemoryDepth = VK_NULL_HANDLE;
		}

		if (vkImageDepth)
		{
			vkDestroyImage(vkDevice, vkImageDepth, nullptr);
			vkImageDepth = VK_NULL_HANDLE;
		}
...
VkResult createRenderPass()
{
	...
	//Attachment reference structure for color
	...
	vkAttachmentReferenceColor.attachment = 0;		//iNDEX IN attachment description arayy
