...
const char* gpSzAppName = "VK-LIGHT";
const wchar_t* windowName = L"LIGHT";
...
//DEPTH IMAGE
VkImage vkImageDepth = VK_NULL_HANDLE;
VkDeviceMemory vkDeviceMemoryDepth = VK_NULL_HANDLE;
VkImageView vkImageViewDepth = VK_NULL_HANDLE;
VkFormat vkFormatDepth = VK_FORMAT_UNDEFINED;
...
VkResult createImageAndImageViews()
{
	...
	if (vkResult != VK_SUCCESS)
	{
		LogData("getSupportedDepthFormat failed!!!");
		return vkResult;
	}

	//For depth image, Initialize VK IMAGE create info
	VkImageCreateInfo vkImageCreateInfo{};
	vkImageCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
	vkImageCreateInfo.pNext = nullptr;
	vkImageCreateInfo.flags = 0;
	vkImageCreateInfo.imageType = VK_IMAGE_TYPE_2D;
	vkImageCreateInfo.format = vkFormatDepth;
	...
	vkResult = vkCreateImage(vkDevice, &vkImageCreateInfo, nullptr, &vkImageDepth);
	if (vkResult != VK_SUCCESS)
	{
		LogData("vkCreateImage depth failed!!!");
		return vkResult;
	}

	//Memory requirements for Depth image
	VkMemoryRequirements vkMemoryRequirements{};
	vkGetImageMemoryRequirements(vkDevice, vkImageDepth, &vkMemoryRequirements);
	...
	for (uint32_t i = 0; i < vkPhysicalDeviceMemoryProp.memoryTypeCount; i++)
	{
		...
		{
			if (vkPhysicalDeviceMemoryProp.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
			{
	...
	vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, nullptr, &vkDeviceMemoryDepth);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed create device memory for depth image!!!");
		return vkResult;
	}

	//3.Bind with device memory handle with vulkan buffer
	vkResult = vkBindImageMemory(vkDevice, vkImageDepth, vkDeviceMemoryDepth, 0);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed bind buffer memory for depth image!!!");
		return vkResult;
	}
	...
	vkImageViewCrateInfo.pNext = nullptr;
	vkImageViewCrateInfo.flags = 0;
	vkImageViewCrateInfo.format = vkFormatDepth;
	//aspectMask:--whch part of Image or whole of the Image is going to be affected by image barrier.
	//https://registry.khronos.org/vulkan/specs/latest/man/html/VkImageSubresourceRange.html
	vkImageViewCrateInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT;
	vkImageViewCrateInfo.subresourceRange.baseMipLevel = 0;
	vkImageViewCrateInfo.subresourceRange.levelCount = 1;
	vkImageViewCrateInfo.subresourceRange.baseArrayLayer = 0;
	vkImageViewCrateInfo.subresourceRange.layerCount = 1;
	...
	vkImageViewCrateInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
	vkImageViewCrateInfo.image = vkImageDepth;


	//creaye image view 
	vkResult = vkCreateImageView(vkDevice, &vkImageViewCrateInfo, nullptr, &vkImageViewDepth);
	if (vkResult != VK_SUCCESS)
	{
		LogData("vkCreateImageView failed!!!");
		return vkResult;
	}
...
VkResult getSupportedDepthFormat()
{
	...
	for (auto& format : formatList)
	{
		...
		vkGetPhysicalDeviceFormatProperties(vkPhysicalDeviceSelected, format, &formatProps);
		if (formatProps.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)
		{
			vkFormatDepth = format;
			break;
		}
	}
...
#pragma region RECORD COMMAND BUFFER
		...
		//here we should call vulkan drawing functions
		{
			...
			vkCmdDraw(vkCommandBuffersVector[i], 36, 1, 0, 0);
		}
...
VkResult createRenderPass()
{
	...
	//For COLOR AND DEPTH, create 2 descriotion vector
	std::vector<VkAttachmentDescription> vkAttachmentDesriptionVector(2);
	...
	//DEPTH
	vkAttachmentDesriptionVector[1].flags = 0;
	vkAttachmentDesriptionVector[1].format = vkFormatDepth;
	vkAttachmentDesriptionVector[1].samples = VK_SAMPLE_COUNT_1_BIT;
	vkAttachmentDesriptionVector[1].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; ////When we enter , what to do about data
	vkAttachmentDesriptionVector[1].storeOp = VK_ATTACHMENT_STORE_OP_STORE; ////When we leave , what to do about data
	vkAttachmentDesriptionVector[1].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
	vkAttachmentDesriptionVector[1].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
	vkAttachmentDesriptionVector[1].initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;	////Image's data(arrangement) when we enter.
	vkAttachmentDesriptionVector[1].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL; ////Image's data(arrangement) when we leave.
	...
	vkAttachmentReferenceDepth.attachment = 1;
	vkAttachmentReferenceDepth.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
	...
	vkSubpassDescription.pDepthStencilAttachment = &vkAttachmentReferenceDepth;
...
VkResult createFrameBuffers()
{
	...
	std::vector<VkImageView> vkImageViewTempVec(2);
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		vkImageViewTempVec[1] = vkImageViewDepth;
...
VkResult createVertexBuffer()
{
	...
	uint32_t vertexBufferSize = static_cast<uint32_t>(cubeVertices.size()) * sizeof(Vertex);
...
VkResult UpdateUniformBuffer()
{
	...
	rotationMatrix = glm::rotate(glm::mat4(rotationMatrix), glm::radians(angle), glm::vec3(1.0f, 1.0f, 0.0f));
	...
	//Push light data
	myUniformData.lightPositon = glm::vec3(1.0f, 1.0f, 0.0f);
	myUniformData.viewPositon = glm::vec3(0.0f, 0.0f, 2.0f);
...
VkResult createDescriptorSetLayout()
{
	...
	vkDescriptorSetBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;    //Add  more with | VK_SHADER_STAGE_VERTEX_BIT
...
VkResult createGraphicsPipeline()
{
	...
	vkVertexInputAttributeDescriptionVector[0].offset = offsetof(Vertex, pos);	//useed in interleaved, how much to jump

	vkVertexInputAttributeDescriptionVector[1].binding = 0;
	...
	vkVertexInputAttributeDescriptionVector[1].offset = offsetof(Vertex, normal);
	...
	vkPipelineDepthStencileStateCreateInfo.flags = 0;
	vkPipelineDepthStencileStateCreateInfo.pNext = nullptr;
