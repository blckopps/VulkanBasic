...
const char* gpSzAppName = "VK-COLOR-TRIANGLE";
const wchar_t* windowName = L"COLOR-TRIANGLE";
...
void UnInitialize(void)
{
	...
	//clean color buffer
	if (vertexDataColor.vkDeviceMemory)
	{
		vkFreeMemory(vkDevice, vertexDataColor.vkDeviceMemory, nullptr);
		vertexDataColor.vkDeviceMemory = VK_NULL_HANDLE;
	}

	if (vertexDataColor.vkBuffer)
	{
		vkDestroyBuffer(vkDevice, vertexDataColor.vkBuffer, nullptr);
		vertexDataColor.vkBuffer = VK_NULL_HANDLE;
	}
...
#pragma region RECORD COMMAND BUFFER
		...
		//here we should call vulkan drawing functions
		{
			...
			vkCmdBindVertexBuffers(vkCommandBuffersVector[i], 0, 1, &vertexDataPosition.vkBuffer, vkDeviceSizeOffsetPositionVector.data());

			std::vector<VkDeviceSize> vkDeviceSizeOffsetColorVector(1);
			vkCmdBindVertexBuffers(vkCommandBuffersVector[i], 1, 1, &vertexDataColor.vkBuffer, vkDeviceSizeOffsetColorVector.data());
...
VkResult createVertexBuffer()
{
	...
	std::vector<float> vertexPositionBuffer
	{
		0.0f, 1.0f, 0.0f,
		-1.0f, -1.0f, 0.0f,
		1.0f, -1.0f, 0.0f
	...
	uint32_t vertexPositionBufferSize = static_cast<uint32_t>(vertexPositionBuffer.size()) * sizeof(vertexPositionBuffer[0]);
	uint32_t vertexColorBufferSize = static_cast<uint32_t>(vertexColorBuffer.size()) * sizeof(vertexColorBuffer[0]);
	VkDeviceSize allocatedDeviceSize = 0;

	//VERTEX POSITION BUFFER
	
	vkResult = createBufferAndMemory(vertexDataPosition, vertexPositionBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, allocatedDeviceSize);
	if (vkResult != VK_SUCCESS)
	{
		LogData("createBufferAndMemory position failed!!!");
		return vkResult;
	}
	...
	//4.Memory map
	void* data = nullptr;
	vkResult = vkMapMemory(vkDevice, vertexDataPosition.vkDeviceMemory, 0, allocatedDeviceSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
	...
	//5.Copy vertex data 
	memcpy(data, vertexPositionBuffer.data(), vertexPositionBufferSize);

	//6.Unmap
	vkUnmapMemory(vkDevice, vertexDataPosition.vkDeviceMemory);

	////////////////////// Vertex Color buffer /////////////////////////////////
	allocatedDeviceSize = 0;

	vkResult = createBufferAndMemory(vertexDataColor, vertexColorBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, allocatedDeviceSize);
	if (vkResult != VK_SUCCESS)
	{
		LogData("createBufferAndMemory color failed!!!");
		return vkResult;
	}

	//.Bind with device memory handle with vulkan buffer
	vkResult = vkBindBufferMemory(vkDevice, vertexDataColor.vkBuffer, vertexDataColor.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed bind color buffer memory!!!");
		return vkResult;
	}

	//4.Memory map
	data = nullptr;
	vkResult = vkMapMemory(vkDevice, vertexDataColor.vkDeviceMemory, 0, allocatedDeviceSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed to map color memory!!!");
		return vkResult;
	}

	//5.Copy vertex data 
	memcpy(data, vertexColorBuffer.data(), vertexColorBufferSize);

	//6.Unmap
	vkUnmapMemory(vkDevice, vertexDataColor.vkDeviceMemory);
...
VkResult UpdateUniformBuffer()
{
	...
	myUniformData.modelMatrix = glm::translate(glm::mat4(1.0), glm::vec3(0.0f, 0.0f, -4.0f));
