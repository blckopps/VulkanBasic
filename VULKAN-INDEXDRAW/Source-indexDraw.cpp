...
VertexData vertexDataPositionIndex{};

//global because, used in display and create vertex buffer
const std::vector<uint32_t> trianglePositionIndicesVector = { 0,1 ,2 };
...
void UnInitialize(void)
{
	...
	//Clean index buffer
	if (vertexDataPositionIndex.vkDeviceMemory)
	{
		vkFreeMemory(vkDevice, vertexDataPositionIndex.vkDeviceMemory, nullptr);
		vertexDataPositionIndex.vkDeviceMemory = VK_NULL_HANDLE;
	}

	if (vertexDataPositionIndex.vkBuffer)
	{
		vkDestroyBuffer(vkDevice, vertexDataPositionIndex.vkBuffer, nullptr);
		vertexDataPositionIndex.vkBuffer = VK_NULL_HANDLE;
	}
...
#pragma region RECORD COMMAND BUFFER
		...
		//here we should call vulkan drawing functions
		{
			...
			//Bind index buffer
			//VK_INDEX_TYPE_UINT32 is natched with the type of 'const std::vector<uint32_t> trianglePositionIndicesVector'
			vkCmdBindIndexBuffer(vkCommandBuffersVector[i], vertexDataPositionIndex.vkBuffer, 0, VK_INDEX_TYPE_UINT32);


			//vkCmdDraw(vkCommandBuffersVector[i], 3, 1, 0, 0);
			unsigned int numIndices = static_cast<unsigned int>(trianglePositionIndicesVector.size()) * sizeof(trianglePositionIndicesVector[0]);

			vkCmdDrawIndexed(vkCommandBuffersVector[i], numIndices, 1, 0, 0, 1);
		}
...
VkResult createVertexBuffer()
{
	...
	uint32_t indexBufferSize = static_cast<uint32_t>(trianglePositionIndicesVector.size()) * sizeof(Vertex);
	...
	vkIndexBufferCreateInfo.pNext = nullptr;
	vkIndexBufferCreateInfo.flags = 0;		//Valid flags are used in scatter buffer.(used in sparse buffers)
	vkIndexBufferCreateInfo.size = indexBufferSize;
	vkIndexBufferCreateInfo.usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT;

	vkResult = vkCreateBuffer(vkDevice, &vkIndexBufferCreateInfo, nullptr, &vertexDataPositionIndex.vkBuffer);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed to create index buffer!!!");
		return vkResult;
	}

	VkMemoryRequirements vkIndexMemoryRequirements{};
	vkGetBufferMemoryRequirements(vkDevice, vertexDataPositionIndex.vkBuffer, &vkIndexMemoryRequirements);

	VkMemoryAllocateInfo vkIndexMemoryAllocateInfo{};
	vkIndexMemoryAllocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	vkIndexMemoryAllocateInfo.pNext = nullptr;
	vkIndexMemoryAllocateInfo.allocationSize = vkIndexMemoryRequirements.size;
	vkIndexMemoryAllocateInfo.memoryTypeIndex = 0;

	for (uint32_t i = 0; i < vkPhysicalDeviceMemoryProp.memoryTypeCount; i++)
	{
		if (1 == (vkIndexMemoryRequirements.memoryTypeBits & 1))
		{
			if (vkPhysicalDeviceMemoryProp.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
			{
				vkIndexMemoryAllocateInfo.memoryTypeIndex = i;
				break;
			}
		}
		vkIndexMemoryRequirements.memoryTypeBits >>= 1;
	}

	//2.Allocate device memory
	vkResult = vkAllocateMemory(vkDevice, &vkIndexMemoryAllocateInfo, nullptr, &vertexDataPositionIndex.vkDeviceMemory);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed create index device memory!!!");
		return vkResult;
	}

	//3.Bind with device memory handle with vulkan buffer
	vkResult = vkBindBufferMemory(vkDevice, vertexDataPositionIndex.vkBuffer, vertexDataPositionIndex.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed bind index buffer memory!!!");
		return vkResult;
	}

	//4.Memory map
	data = nullptr;
	vkResult = vkMapMemory(vkDevice, vertexDataPositionIndex.vkDeviceMemory, 0, vkIndexMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		LogData("Failed to map index memory!!!");
		return vkResult;
	}

	//5.Copy vertex data 
	memcpy(data, trianglePositionIndicesVector.data(), indexBufferSize);

	//6.Unmap
	vkUnmapMemory(vkDevice, vertexDataPositionIndex.vkDeviceMemory);
